...
def crear_run(proyecto_id: str, _body: RunCreate | None = None, db: Session = Depends(get_db)):
...
def process_next_item(run_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        sim_avg, hits, val_score = validate_breach_with_rag(db, art.id, res.get("brecha", ""))
        ent_bits, ent_norm = shannon_entropy_bits_and_norm(res.get("brecha", ""))
        dup_row, es_dup = find_duplicate_breach(db, art.id, res.get("brecha", ""), thr=0.80)
        estado_val, razon_val = auto_validate(sim_avg, ent_norm, val_score, es_dup)
