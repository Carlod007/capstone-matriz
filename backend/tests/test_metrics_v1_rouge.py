...
@pytest.mark.bd
class TestPromedios:
    def test_sin_pares_comparables_el_promedio_no_es_cero(self, db, usuario_prueba):
        """Un cero se lee como "los resumenes no se parecen en nada", y lo que
        ocurre es que no habia nada comparable. Debe venir sin valor."""
        import uuid
        ...
        db.add(Proyecto(id=pid, usuario_id=usuario_prueba["id"],
                        ...
                        objetivo="Comprobar el promedio de ROUGE en v1",
        ...
        db.add(ResultadoResumen(id=str(uuid.uuid4()), articulo_id=aid,
                                resumen_generado=RESUMEN_ES,
                                resumen_referencia=ABSTRACT_EN))
        db.commit()
